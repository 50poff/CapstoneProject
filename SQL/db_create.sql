...
CREATE TABLE IF NOT EXISTS `DaiHire`.`Meeting` (
  ...
  `meeting_time` TIME NOT NULL COMMENT '\"time\" is an oracle keyword so I made the column \"meeting_time\"',
...
CREATE TABLE IF NOT EXISTS `DaiHire`.`PeopleAvailability` (
  ...
  PRIMARY KEY (`people_id`, `timeslot`),
  INDEX `fk_PeopleAvailability_timeslot1_idx` (`timeslot` ASC),
  ...
  CONSTRAINT `fk_PeopleAvailability_timeslot1`
    ...
    REFERENCES `DaiHire`.`timeslot` (`timeslot_datetime`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DaiHire`.`CandidateAvailability` (
  `candidate_id` INT NOT NULL,
  `timeslot` DATETIME NOT NULL,
  PRIMARY KEY (`candidate_id`, `timeslot`),
  INDEX `fk_CandidateAvailability_timeslot1_idx` (`timeslot` ASC),
